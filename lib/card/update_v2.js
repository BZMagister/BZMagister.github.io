...
options = {
	...
	evo_level: [1, 2, 3, 4, 5],
	senzai: ["攻擊力上升Ⅰ", "攻擊力上升Ⅱ", "elmts", "全屬性攻擊力上升Ⅰ", "HP上升Ⅰ", "HP上升Ⅱ", "elmts", "全屬性HP上升Ⅰ", "全屬性HP上升Ⅱ", "減輕火屬性傷害Ⅰ", "減輕水屬性傷害Ⅰ", "減輕雷屬性傷害Ⅰ", "減輕火、水屬性傷害Ⅰ", "減輕火、雷屬性傷害Ⅰ", "減輕水、雷屬性傷害Ⅰ", "減輕全屬性傷害Ⅰ", "elmts", "減少COSTⅡ", "快速技能Ⅰ", "九死一生Ⅰ"],
	evo_material: ["none"]
};
					...
					$("<td>").append(
						$('<select>').attr('id', key[0]).append(function() {
							...
							if ("evo_material" === key[3]) {
								...
								htmlOptions = htmlSenzaiOptions(card_data[key[0]], card_data.prop, card_data.breed);
							}
							else {
 ...
 * @param  {string} cardSenzaiVal  The value of card_data.senzai_i.
 * @param  {string} changedProp    New Prop. of some changing.
 * @param  {string} changedBreed   New Breed of some changing.
 * @return {HTML}                  Options in HTML format.
 */
function htmlSenzaiOptions(cardSenzaiVal, changedProp, changedBreed) {
	if ( undefined === evo_material[changedProp] )
	...
	for (var option_key in options.senzai) {
		...
		if ( undefined === cardSenzaiVal || option != cardSenzaiVal )
			htmlOptions.after($('<option>').html(option));
		else
			htmlOptions.after($('<option>').html(option).attr('selected', true));
	}
